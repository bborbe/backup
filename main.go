...
func (a *application) createBackupCron(
	...
	return factory.CreateBackupCron(
		sentryClient,
		backupExectuor,
		backupClientset,
		apiextensionClientset,
...
func (a *application) createCleanupCron(
	sentryClient libsentry.Client,
	backupCleaner pkg.BackupCleaner,
	backupClientset pkg.BackupClientset,
	apiextensionClientset libk8s.ApiextensionsInterface,
) run.Func {
	return factory.CreateCleanupCron(
		sentryClient,
		backupCleaner,
		backupClientset,
		apiextensionClientset,
...
func (a *application) createSetupResourceDefinition(
	backupClientset pkg.BackupClientset,
	apiextensionClientset libk8s.ApiextensionsInterface,
	trigger run.Trigger,
) func(ctx context.Context) error {
	return factory.CreateSetupResourceDefinition(
		backupClientset,
		apiextensionClientset,
		k8s.Namespace(a.Namespace),
		trigger,
...
func (a *application) createHttpServer(
	...
	return func(ctx context.Context) error {
		...
		router.Path("/status").
			Handler(factory.CreateStatusHandler(backupClientset, apiextensionClientset, libk8s.Namespace(a.Namespace), pkg.Path(a.BackupRootDir)))
		router.Path("/list").
			Handler(factory.CreateListHandler(backupClientset, apiextensionClientset, libk8s.Namespace(a.Namespace)))
		router.Path("/backup/all").
			Handler(factory.CreateBackupActionHandler(ctx, sentryClient, backupExectuor, backupClientset, apiextensionClientset, libk8s.Namespace(a.Namespace)))
		router.Path("/backup/{name}").
			Handler(factory.CreateBackupHandler(backupClientset, apiextensionClientset, libk8s.Namespace(a.Namespace), backupExectuor))
		router.Path("/cleanup/all").
			Handler(factory.CreateCleanActionHandler(ctx, sentryClient, backupCleaner, backupClientset, apiextensionClientset, libk8s.Namespace(a.Namespace)))
		router.Path("/cleanup/{name}").
			Handler(factory.CreateCleanupHandler(backupClientset, apiextensionClientset, libk8s.Namespace(a.Namespace), backupCleaner))
